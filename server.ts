...
const chatsCollection = "chatMessages";
const blocksCollection = "chatBlocks";
...
async function startServer() {
  ...
  // In-memory data removed. Data is fetched from Firebase Firestore.
...
لا تقم بابتكار معلومات اتصال بديلة أو خدمات لا نقوم بتقديمها. لا تضف أي إعلانات تجارية خارجية.`;
  ...
  app.get("/api/chats/blocks", async (req, res) => {
    try {
      const blockedUsers = await getDocsRest(blocksCollection);
  ...
  app.post("/api/chats/block", async (req, res) => {
    ...
    try {
      const blockedUsers = await getDocsRest(blocksCollection);
      const exists = blockedUsers.some((b: any) => b.blockerPhone === blockerPhone && b.blockedPhone === blockedPhone);
      
      if (!exists) {
        await addDocRest(blocksCollection, { blockerPhone, blockedPhone });
      }
      const updatedBlocks = await getDocsRest(blocksCollection);
      res.json({ success: true, blockedUsers: updatedBlocks });
    } catch (error) {
      res.status(500).json({ error: "فشل الحظر" });
    }
  });

  app.post("/api/chats/unblock", async (req, res) => {
    const { blockerPhone, blockedPhone } = req.body;
    if (!blockerPhone || !blockedPhone) {
    ...
    try {
      const blockedUsers = await getDocsRest(blocksCollection);
      const toDelete = blockedUsers.filter((b: any) => b.blockerPhone === blockerPhone && b.blockedPhone === blockedPhone);
      
      for (const b of toDelete) {
        await deleteDocRest(blocksCollection, b.id);
      }
  ...
  app.get("/api/chats", async (req, res) => {
    ...
    try {
      const allMessages = await getDocsRest(chatsCollection);
      if (!phone) {
        return res.json(allMessages);
      }
      // Filter messages where sender or receiver is this phone
      const filtered = allMessages.filter(
        (msg: any) => msg.senderPhone === phone || msg.receiverPhone === phone
      );
      // Sort by timestamp
  ...
  app.post("/api/chats/read", async (req, res) => {
    ...
    try {
      ...
      const unreadMessages = allMessages.filter(
        (msg: any) => msg.senderPhone === partnerPhone && msg.receiverPhone === myPhone && msg.isRead !== true
      );

      for (const msg of unreadMessages) {
        await updateDocRest(chatsCollection, msg.id, { isRead: true });
      }

      res.json({ success: true });
    } catch (error) {
      res.status(500).json({ error: "فشل تحديث الرسائل المقروءة" });
    }
  });
  ...
  app.post("/api/chats", async (req, res) => {
    ...
    try {
      ...
      const blockedUsers = await getDocsRest(blocksCollection);
      const isBlockedByMe = blockedUsers.some((b: any) => b.blockerPhone === senderPhone && b.blockedPhone === receiverPhone);
      const isBlockedByThem = blockedUsers.some((b: any) => b.blockerPhone === receiverPhone && b.blockedPhone === senderPhone);
      ...
      const docId = await addDocRest(chatsCollection, newMsg);
      const insertedMsg = { id: docId, ...newMsg };

      // Trigger simulated advertiser auto-reply
      setTimeout(async () => {
        try {
          // Mark user's message as read since the advertiser has opened it to reply
          await updateDocRest(chatsCollection, docId, { isRead: true });
          ...
          if (adId) {
            try {
              ...
              if (adId.startsWith("ad-") || !adId.startsWith("market-")) {
                  jadDoc = await getDocRest(jobsCollection, adId);
              }
              if (!jadDoc && (adId.startsWith("market-") || !adId.startsWith("ad-"))) {
                  madDoc = await getDocRest(marketCollection, adId);
              }
...
4. أعد نص الرد مباشرة فقط دون أي كلام آخر أو علامات اقتباس أو تعليقات.`;
          ...
          await addDocRest(chatsCollection, autoReplyMsg);
        } catch (err) {
          console.error("Error in triggering advertiser auto-reply:", err);
        }
      }, 2000);

      res.json({ success: true, message: insertedMsg });
    } catch (err) {
